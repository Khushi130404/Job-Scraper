...
query = "it-jobs"
base_url = f"https://www.linkedin.com/{query}?src=gnbjobs_homepage_srch"
driver.get(base_url)
...
while True:
    ...
    # Find all job titles
    titles = soup.select("div.job-title a")  #
    titles_list = [title.getText().strip() for title in titles]

    # Find Company Names
    company_names_list = [comp.getText().strip() for comp in soup.select("span.company-name")]  #
    ...
    experiences = [exp.getText().strip() for exp in soup.select("span.exp")]  #
    ...
    # Find salaries
    salaries = [sal.getText().strip() for sal in soup.select("div.salary")]  #
...
# Save data to Excel
excel_file = "job_linkedIn.xlsx"
df = pd.DataFrame(all_data)
df.to_excel(excel_file, index=False)

# Save data to JSON
json_file = "job_linkedIn.json"
